...
const PartnerForm = dynamic(() => import("../forms/PartnerForm"), { loading: () => <p>Loading...</p> });
...
const PartnerModal = ({
  ...
  const handleSuccess = () => {
    ...
    if (onSuccess) {
      onSuccess();
      const messages = {
        create: "Partenaire ajouté avec succès",
        edit: "Partenaire modifié avec succès",
  ...
  const handleDelete = async () => {
    ...
    try {
      const response = await fetch(`http://localhost:8000/partners/delete/${id}`, {
        method: 'DELETE',
        headers: {
      ...
      showToast("Une erreur est survenue lors de la suppression du partenaire", 'error');
  ...
  const handleBan = async () => {
    ...
    try {
      const response = await fetch(`http://localhost:8000/partners/ban/${id}`, {
        method: 'PATCH',
        headers: {
          'Authorization': `Bearer ${localStorage.getItem('token')}`,
          'Content-Type': 'application/json',
    ...
    if (type === "delete" && id) {
      return (
        <form onSubmit={(e) => { e.preventDefault(); handleDelete(); }} className="p-4 flex flex-col gap-4">
          <span className="text-center font-medium">
            Toutes les données seront perdues. Êtes-vous sûr de vouloir supprimer ce partenaire ?
          </span>
          {error && (
      ...
      return (
        <form onSubmit={(e) => { e.preventDefault(); handleBan(); }} className="p-4 flex flex-col gap-4">
          <span className="text-center font-medium">
            {data.isBanned 
              ? "Voulez-vous débannir ce partenaire ?" 
              : "Voulez-vous bannir ce partenaire ?"}
          </span>
          {error && (
            <p className="text-red-500 text-sm text-center">{error}</p>
          )}
          <div className="flex gap-2 justify-center">
            ...
            <button
              ...
              disabled={loading}
            >
              {loading ? "En cours..." : (data.isBanned ? "Débannir" : "Bannir")}
            </button>
          </div>
        </form>
      );
