...
const DestinationModal = ({ type, data, id, onSuccess }: Props) => {
  ...
  const handleSuccess = () => {
    ...
    onSuccess?.();
    const messages = {
      create: "Destination ajoutée avec succès",
      edit: "Destination modifiée avec succès",
      delete: "Destination supprimée avec succès"
    };
  ...
  const handleDelete = async () => {
    ...
    try {
      const response = await fetch(`http://localhost:8000/destination/delete/${id}`, {
        method: 'DELETE',
        headers: {
      ...
      return (
        <form onSubmit={(e) => { e.preventDefault(); handleDelete(); }} className="p-4 flex flex-col gap-4">
          <span className="text-center font-medium">
            Êtes-vous sûr de vouloir supprimer cette destination ? Toutes les données associées seront perdues.
          </span>
