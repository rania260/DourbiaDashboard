...
const CircuitModal = ({ type, data, id, onSuccess }: Props) => {
  ...
  const handleSuccess = () => {
    ...
    onSuccess?.();
    const messages = {
      create: "Circuit ajouté avec succès",
      edit: "Circuit modifié avec succès",
      delete: "Circuit supprimé avec succès"
    };
  ...
  const handleDelete = async () => {
    ...
    try {
      const response = await fetch(`http://localhost:8000/circuit/delete/${id}`, {
        method: 'DELETE',
        headers: {
      ...
      return (
        <form onSubmit={(e) => { e.preventDefault(); handleDelete(); }} className="p-4 flex flex-col gap-4">
          <span className="text-center font-medium">
            Êtes-vous sûr de vouloir supprimer ce circuit ? Toutes les données associées seront perdues.
          </span>
